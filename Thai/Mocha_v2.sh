...
cp $HOME/go/bin/celestia-appd $HOME/.celestia-app/cosmovisor/genesis/bin
}
...
function CustomPort  {
echo " "
sudo sed -i '/Celestia_PORT/d' $HOME/.bash_profile
echo -e "\e[1m\e[32mCustom your Port... \e[0m" && sleep 1
while true; do
  read -p "Insert Port 10-55: " CELESTIA_PORT
  if [[ $CELESTIA_PORT =~ ^[1-9]?[0-9]$ ]] && [ $CELESTIA_PORT -ge 10 ] && [ $CELESTIA_PORT -le 55 ]; 
then
...
sed -i.bak -e "s%^address = \"tcp://0.0.0.0:1317\"%address = \"tcp://0.0.0.0:${Celestia_PORT}17\"%; s%^address = \":8080\"%address = \":${Celestia_PORT}80\"%; s%^address = \"0.0.0.0:9090\"%address = \"0.0.0.0:${Celestia_PORT}90\"%; s%^address = \"0.0.0.0:9091\"%address = \"0.0.0.0:${Celestia_PORT}91\"%; s%^address = \"0.0.0.0:8545\"%address = \"0.0.0.0:${Celestia_PORT}45\"%; s%^ws-address = \"0.0.0.0:8546\"%ws-address = \"0.0.0.0:${Celestia_PORT}46\"%" $HOME/.celestia-app/config/app.toml
sed -i.bak -e "s%^node = \"tcp://localhost:26657\"%node = \"tcp://localhost:${Celestia_PORT}657\"%" $HOME/.celestia-app/config/client.toml 
}
...
options=("Install" "Install + Snap" "Install + Snap with old wallet" "Check Sync" "Snapshort" "Custom Port" "Check Balance" "Create Validator" "Restart" "Delegate" "Uninstall" "Add Peer" "Quit")
...
CustomPort
echo -e "\e[1m\e[32mYour Node was Custom Port complete!\e[0m" && sleep 1
break
;;
